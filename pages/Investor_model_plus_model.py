...
def prediction(profil_investisseur, Married, ApplicantIncome, Loan_Amount_Term, Credit_History):   
 
    # Pre-processing user input    
    if profil_investisseur == "prudent":
        profil_investisseur = 0

    elif profil_investisseur =="stable":
        profil_investisseur = 1

    else:
        profil_investisseur ==2
    ...
    # Making predictions 
    prediction = classifier.predict([[profil_investisseur]])
     
    if prediction == 0:
        pred = 'Personne stable'
        
    else:
        pred = 'Personne Agressif'
    return pred

############################### PARTIE DU CODE POUR R ##################################################################################################################################################################################################
...
def main():
    ...
    if send_button:
        ...
        #st.balloons()
        st.snow()
    prediction_button = st.button("Effectuer la prediction du profil investisseur")
    if prediction_button:
        profil_investisseur, total = calculer_profil_investisseur(reponses)
        result = prediction (profil_investisseur)
        st.success("Your loan is {}".format(result))
        st.success("Les données ont été enregistrées avec succès!")
        st.snow
        #prediction(profil_investisseur)
